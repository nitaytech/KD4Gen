...
from src.kd_models_accelerator_multiple_loaders import train_kd
from src.kd_utils import LossScheduler
from src.constants import *


def prepare_loss_scheduler(args) -> LossScheduler:
    with open("arg_configs/kd_experiment_configs.yaml", 'r') as f:
        all_experiment_configs = yaml.safe_load(f)
    experiment_config = all_experiment_configs[args.experiment_config_key]
    layer_kwargs = all_experiment_configs[experiment_config['layer']]
    list_of_loss_kwargs = [all_experiment_configs[config_id] for config_id in experiment_config['losses']]
    for loss_kwargs in list_of_loss_kwargs:
        loss_kwargs.update(layer_kwargs)
    scheduling_kwargs = all_experiment_configs[experiment_config['scheduling']]
    if experiment_config['scheduling'] == 'stage_scheduling':
        scheduling_kwargs['change_every_n_epochs'] = max(1, args.epochs // len(list_of_loss_kwargs))
    elif experiment_config['scheduling'] == 'patience_epochs_scheduling':
        scheduling_kwargs['change_every_n_epochs'] = args.n_patience_epochs
    loss_scheduler = LossScheduler(list_of_loss_kwargs=list_of_loss_kwargs, **scheduling_kwargs)
    return loss_scheduler
...
def main(args):
    ...
    return train_kd(output_dir=args.output_dir, loss_scheduler=loss_scheduler,
                    datasets=datasets, student_name=args.student_name, teacher_name=args.teacher_name,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher_name', type=str, default=TEACHER_MODEL_NAME, help='Name of the teacher model.')
    parser.add_argument('--student_state_dict_path', type=str, default=None,
                        help='Path to the student model state dict.')
    parser.add_argument('--teacher_state_dict_path', type=str, default=None,
                        help='Path to the teacher model state dict.')
